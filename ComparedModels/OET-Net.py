...
def bce_loss(y_true, y_pred):
    params = tf1.count_nonzero(y_true[0])/262144
    alpha = tf.where(params<0.01, (0.01-params)*100, 1)
    ...
    return alpha*(loss0 + loss1 + loss2 + loss3 + loss4 + loss5 + loss6)
...
class myNet(keras.models.Model):
    ...
    def call(self, inputs):
        ...
        fused_output = self.conv10(tf.concat([side1, side2, side3, side4, side5, side6], axis=3))
...
def train():
    ...
    print('Fitting model...')
    model.fit(imgs_train, imgs_mask_train, batch_size=1, epochs=40, verbose=1, shuffle=True,
              callbacks=[model_checkpoint])
    # model.save('myu2net.h5')
    print('predict test data')
    i = 0
    piclist = []
    for line in open("./results6/pic.txt"):
        line = line.strip()
